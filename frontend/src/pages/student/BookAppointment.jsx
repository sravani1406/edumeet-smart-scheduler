...
const BookAppointment = () => {
  ...
  const fetchTeacherData = async () => {
    try {
      ...
      setTeacher(
        teacherRes.data.name ? teacherRes.data : teacherRes.data.teacher
      );
      ...
      if (availabilityData && availabilityData.slots) {
        const convertedAvailability = Object.keys(availabilityData.slots)
          .filter(
            day =>
              availabilityData.slots[day] &&
              availabilityData.slots[day].length > 0
          )
          .map(day => ({
            dayOfWeek: day,
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      toast.success(
        'Appointment booked successfully! Waiting for teacher confirmation.'
      );
    ...
    <div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          {/* ✅ RESTORED WEEKDAY DISPLAY */}
          {availability.length > 0 && (
            <div className="mt-4 p-4 bg-blue-50 rounded-lg">
              <h3 className="font-semibold text-sm mb-2">
                Teacher's Available Days:
              </h3>
                ...
                {availableSlots.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {availableSlots.map((slot, index) => {
                      const convertedStart = convertTo24Hour(slot.startTime);

                      const isBooked = bookedSlots.some(
                        booked => booked.startTime === convertedStart
                      );
